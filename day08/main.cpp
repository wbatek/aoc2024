...
int solution(std::vector<std::vector<char>> lines, std::unordered_map<char, std::vector<std::pair<int, int>>>& positions, int range) {
    int count = 0;
...
int main() {
    ...
    int result_1 = solution(lines, antenna_positions, 1);
    writeResultToFile(result_1, 1);

    int result_2 = solution(lines, antenna_positions, lines.size());
